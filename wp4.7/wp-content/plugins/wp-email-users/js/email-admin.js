 ...
 weu('.wau_user_toggle').show();
  ...
  function validation_smtp() {
    ...
    if (document.layers||document.getElementById||document.all)
      ...
      document.getElementById('errfn').innerHTML="";
    return true
    var inputtxt = document.wau_smtp_form.weu_smtp_name;
    ...
    {
      swal("Email from name", "Please enter alphanumerics only!", "error");
      return false;
    }
...
function validation() {
  ...
  var inputtxt = document.myform.wau_from_name;
...
function popup_template() {
  weu(".template_prev_class").click(function() {
    var id = this.id;
    var temp_id = weu("textarea[name=wau_mailcontent_"+id+"]").val();
    weu("#template_value").html(temp_id);
  });    
}
