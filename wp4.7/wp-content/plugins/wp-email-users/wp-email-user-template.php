...
function weu_template(){
	...
	echo '<tr>';
	echo '<th>Select Option</th><td colspan="1"><label><input type="radio" name="template_radio" value="1" id="clear_content" onclick="checktemplate()"> Create Own</label>
	...
	echo '<th>Template</th><td colspan="1"><select autocomplete="off" id="wau_template_single" name="mail_template[]" onchange="jsFunction()" class="wau-template-selector" style="width:100%; height: 50px ">
	...
	echo '<th>Save as New?</th><td colspan="1"><label><input type="radio" name="new_template" value="1" checked="checked" onclick="check_new_template()">Yes</label><label><input type="radio" name="new_template" value="2" onclick="check_new_template()">No</label></td></tr>';
	...
	if($count >= 1){
		...
		<tbody>';
			...
			foreach ( $myrows as $user ){
				...
				wp_editor($user->template_value, "wau_mailcontent_".$user->id,array('wpautop'=>false,'media_buttons' => true));

				echo '</div>';
				echo '<button name="'.$user->id.'" id="'.$user->id.'" class="template_prev_class" data-popup-open="popup-1" ><span class="dashicons dashicons-visibility" onclick="popup_template()"></span></button></form></td>';
				
				echo '<td><form action="" method="post">'; 
				wp_nonce_field( "delete_smtp", "delete_smtp_nonce" );
				echo '<button class="delete-temp-conf" name="'.$user->id.'" type="submit" value="" ><span class="dashicons dashicons-trash"></span></button><input type="hidden" name="submit-delete" value="'.$user->id.'" ></form></td>';
				echo '</tr>';
			}
	...
	if(isset($_POST['submit-delete']))
	{
		if ( ! isset( $_POST['delete_smtp_nonce'] ))
		{
			...
			$table_name = $wpdb->prefix.'email_user';
			$local_weu_sent_id = sanitize_text_field($_POST['submit-delete']);
